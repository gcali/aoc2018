...
export function applyPattern(elements: number[], pattern: Pattern) {
    return elements.map((_, position) => Math.abs(elements.map((element, index) => ({ element, index })).reduce((acc, next) => (acc + next.element * pattern.get(next.index, position)), 0) % 10));
}

export async function applyPatternIteratively(elements: number[], pattern: Pattern, howManyTimes: number, debug?: (n: number) => Promise<void>) {
    let current = 0;
...
export class Pattern {
    constructor(private basePattern: number[]) {
    ...
    public get(index: number, position: number): number {
        ...
        return this.basePattern[realIndex % basePattern.length];
    }
    ...
    public get length() {
        return this.basePattern.length;
    }
}
...
export const flawedFrequencyTransmission = entryForFile(
    ...
    async ({ lines, outputCallback, pause, isCancelled }) => {
        ...
        const result = await applyPatternIteratively(repeatedInput, Pattern.default(), 100, async n => await outputCallback("Iteration " + n + " done"));
