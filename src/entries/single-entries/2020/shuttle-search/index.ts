...
export const shuttleSearch = entryForFile(
    async ({ lines, outputCallback, resultOutputCallback }) => {
        ...
        const result = ids.map(id => ({id: id, ts: id - (ts % id)})).reduce((acc, next) => acc.ts < next.ts ? acc : next);
    ...
    async ({ lines, outputCallback, resultOutputCallback }) => {
        ...
        const toSolve: {a: bigint; c: bigint; y: bigint}[] = [];
    ...
    { key: "shuttle-search", title: "Shuttle Search", embeddedData: true, stars: 2, supportsQuickRunning: true}
);